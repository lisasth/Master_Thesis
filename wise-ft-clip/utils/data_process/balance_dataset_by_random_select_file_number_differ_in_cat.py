...
def get_priority(num_dict):
    ...
    for dataset_name, num in num_dict.items():
        ...
        priority = None
        if "2024_07" in dataset_name_lower and "sco" in dataset_name_lower:
            priority = 1
...
def calculate_num_each_pack_type(data_dict):
    if "no_pack_type" in data_dict:
        for pack_name, pack_dict in data_dict.items():
            pack_dict["total_images_needed"] = sum(pack_dict.values())
    else:
        num_each_pack_list = [sum(pack_dict.values()) for pack_dict in data_dict.values()]
        num_each_pack_list = sorted(num_each_pack_list)
        min_num = min(num_each_pack_list)
        if min_num < 100:
            total_images_needed = num_each_pack_list[1]
        else:
            total_images_needed = min_num
        for pack_name, pack_dict in data_dict.items():
            pack_dict["total_images_needed"] = total_images_needed


if __name__ == "__main__":
    root = "/home/jingjie/wise-ft-clip/data/Train_UK_cleaned"
    dst_root = "/home/jingjie/wise-ft-clip/data/Train_UK_cleaned_balanced_pack_in_each_class"
    path_to_class_list = "/home/jingjie/wise-ft-clip/class_names_list/classes_2024_08_16_uk_hersham.txt"
    ...
    for class_name in class_list:
        ...
        calculate_num_each_pack_type(pack_type_to_dataset_name_to_num_files)
        for pack_name, pack_dict in pack_type_to_dataset_name_to_num_files.items():
            total_images_needed = pack_dict.pop("total_images_needed")
            print("------------------------------------")
