...
def train_classification_head_on_efficientnet_backbone(args):
    ...
    # Load models
    finetuned = utils.torch_load(args.load)
    backbone = finetuned.features
    avgpool = finetuned.avgpool
    input_dim = (list(backbone.children())[-1]).out_channels
