...
def combine_mlp_heads(class_heads_list):
    ...
    hidden_dims = [single_hidden_dim * len(class_heads)]
    output_dim = len(class_heads)
    combined_class_heads = MLPHead(input_dim, hidden_dims, output_dim)
    with torch.no_grad():
...
def combine_classification_heads(args):
    ...
    finetuned.classifier[-1] = combined_class_head
    combined_image_classifier = finetuned
