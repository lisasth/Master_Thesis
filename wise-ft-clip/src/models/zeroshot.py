...
def get_zeroshot_classifier(args, clip_model):
    ...
    if args.classnames_to_encode is not None:
        classnames_to_encode = []
        with open(args.classnames_to_encode, "r") as f:
            for line in f:
                classnames_to_encode.append(line.strip())
    else:
        classnames_to_encode = dataset.classnames
    print(f"classnames_to_encode is {classnames_to_encode}")
    
    print('Getting zeroshot weights.')
    with torch.no_grad():
        zeroshot_weights = []
        for classname in tqdm(classnames_to_encode):
            texts = []
            for t in template:
